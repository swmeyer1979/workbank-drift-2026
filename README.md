...
`streamlit_app.py` provides a local data-product view over the derivative CSV outputs: occupation filter, migration filter, near-threshold filter, zone counts, migration counts, and task drilldown.
...
The verifier imports helper functions without requiring Streamlit. Runtime app launch requires Streamlit installed in the active environment.
...
- **Agentic spot-check:** ~20 sampled tasks are attempted end-to-end by Perplexity Comet K2.6 + tool-use, and actual completion quality is compared to the predicted score.
...
- **Self-grading prior:** raters score tasks that members of their own model family perform. Family caps (physical → ≤2, sustained interpersonal → ≤3, safety-critical unsupervised → ≤3) mitigate the upper tail; the score-3-vs-4 boundary, where most drift sits, is not externally validated. The agentic spot-check is the partial mitigation; expanded independent grading remains the next step.
...
- **Benchmark anchors are decorative in places:** ~50% of cells in `benchmarks.yaml` are null; all populated cells are flagged `status: unverified` (extracted from third-party aggregators, not vendor model cards). Audit of full_844 outputs shows Kimi K2.6 cited an empty anchor list on ~72% of calls, indicating one of three raters is essentially unanchored — a silent failure mode flagged in [`reports/anchor_audit.md`](./reports/anchor_audit.md).
...
- **Reproducibility under model drift:** the headline percentage is a snapshot timestamped 2026-07-07. Models released after this date will produce different numbers under the same protocol.
...
Credentials: see [`.env.example`](./.env.example). Codex OAuth GPT-5.5 xhigh uses local Codex OAuth (`~/.codex/auth.json`). OpenAI GPT-5.5 requires `OPENAI_API_KEY`. Kimi K2.6 and Grok 4.20 are routed via OpenRouter (`OPENROUTER_API_KEY`).

> **Ensemble note (2026-07-07):** Primary rerun uses `codex_gpt_5_5_xhigh,kimi_k2_6,gpt_5_5`. Grok 4.20 is sensitivity only. Archived Gemini and failed Opus smoke output are excluded from refreshed primary aggregation.
